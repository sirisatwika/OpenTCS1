...
public class StandardRemoteVehicleService
  ...
  /*@Override
  public Vehicle createVehicle(ClientID clientId,VehicleCreationTO to) {
    userManager.verifyCredentials(clientId, UserPermission.READ_DATA);

    return vehicleService.createVehicle(to);
  }*/
  ...
  @Override
  public void updateVehicleTemperature(ClientID clientId,
                                            TCSObjectReference<Vehicle> ref,
                                            int temp)
      throws RemoteException {
    userManager.verifyCredentials(clientId, UserPermission.MODIFY_VEHICLES);
    ...
    try {
      kernelExecutor.submit(
          () -> vehicleService.updateVehicleTemperature(ref, temp)
      ).get();
    }
    catch (InterruptedException | ExecutionException exc) {
      throw findSuitableExceptionFor(exc);
